...
func init() {
	logPrefix := "transcript-qa-complete.main.init(),"
...
func main() {
	...
	// log.Printf("Enter transcript-ga.main, Config: %+v\n", Config)

	// set ServiceName and QueueName appropriately
	prefix := "TaskTranscriptQAComplete"
...
func taskHandler(a adding.Service) httprouter.Handle {
	...
	return func(w http.ResponseWriter, r *http.Request, p httprouter.Params) {
		...
		// The QA'd, highest-confidence transcript becomes FinalTranscript
		...
		if duration, err = newRequest.AddTimestamps("BeginTranscriptionQAComplete", startTime, "EndTranscriptionQAComplete"); err != nil {
		...
		output := fmt.Sprintf("%s.taskHandler completedin %v: queue %q, task %q, newRequest: %+v",
